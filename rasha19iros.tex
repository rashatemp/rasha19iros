...
\title[Gradient-based Sample Selection for Transfer Learning of Semantic Segmentation]{Gradient-based Sample Selection for Transfer Learning of\\ Semantic Segmentation: Application in Agricultural Robotics}
...
\author[Submission ID]{\parbox{\textwidth}{\centering Submission ID\thanks{Acknowledgement}}
\\
{\parbox{\textwidth}{\centering  paper1038}
}
}
...
We evaluated our approach on two challenging  datasets and show that we achieve a higher accuracy with a  smaller
...
% I think the following is slightly too much detail for the introduction; we might move it to a later part of the paper:
...
% re-trained on  annotated images taken in the new field.

% Our approach selects samples in batches, each time refining the network, then computing a
...
To compute the measures explained in the following sections, we use what we refer to in the text as pseudo ground truth. These are foreground masks generated by clustering with very weak supervision. An example is shown in figure \ref{fig:unsupervised_foreground}. We detail this further in the subsequent section.
...
We note that the pseudo ground truth is only used to compute the loss but the network weights remain unchanged. They are only later updated with the manual annotations of the selected samples.
...
We then sort the images based on the computed uncertainty measure in a descending order and pick the images accordingly to refine the network on a new dataset. The images are selected on a log-space scale as mentioned in Section \ref{sec:approach}.
...
%Samples selected by the gradient norm are shown in yellow; ones selected by the gradient projection method are shown in red. The plots illustrate that both approaches achieve diversity despite the fact that the strongest gradients cluster together, near the bottom left. However, the fact that the gradient projection method selects many points at the boundary of the distribution suggests that it might be improved further by adding a mechanism to ensure that selected images are representative of a larger subset in the overall dataset. We plan to investigate this in our future work.
 ...
 %          \vspace{1em}
    ...
    \end{table}
        ...
        \begin{table}
        \centering
        \caption{Object-wise Performance on the Zurich dataset. Each row shows the performance after selecting 10 samples with the different methods and refining the network. Running the model without any new annotations yields an accuracy of $0.33$.}
        \begin{tabular}{@{}cccccc@{}} 
            \toprule
            \makecell{Samples \\ No.} & \makecell{Random} & \makecell{Uncertainty} & \makecell{Loss} & \makecell{Gradient \\ Norm} & \makecell{Gradient \\ Proj.} \\ 
            \midrule 
