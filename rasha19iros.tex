...
We then sort the images based on the computed confidence measure and select samples from the ranking evenly on a log scale, to achieve diversity while giving preference to ones with higher uncertainty. The selected images and their manual segmentations are used to refine the network for use on a new dataset. In a pilot experiment, the log scale selection was found to yield better results than selecting only samples with the highest uncertainty. It is used in the following methods as well.
...
%    \vspace{1em} 
    \centering
    \includegraphics[width=\linewidth]{pics/tsne_grad_projection.pdf}
   		\caption{t-SNE of the images gradients on the Stuttgart dataset. The first 10 samples selected by the gradient projection method are shown in red.}
		\label{fig:tsne_proj}    		
 %   \vspace{1em}
   \end{figure}
