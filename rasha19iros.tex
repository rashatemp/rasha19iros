...
approach in the domain for automatic weed control for agricultural robots,
reducing the annotation efforts when moving to different field or environmental
conditions.

In our approach, we compute pseudo ground truth labels using  unsupervised 
segmentation then use those labels to estimate how new, unlabeled samples will 
affect the weights of the CNN if selected for training. We select the training
samples for user annotation based on the effect on the weights and refine the 
network. We evaluated and verified the performance gain of our gradient-based 
approach on two agricultural robotics datasets for weed detection. The datasets 
reveal different characteristics from the data on which the network was pretrained.
Our results show the effectiveness of our method as it produces higher
