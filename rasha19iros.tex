...
new  ranking of the remaining unlabeled data. The best samples are then selected and the
network is refined again.  We
...
higher accuracy on both datasets with a lower  number of samples compared to
...
For the methods presented in sections \ref{sec:loss}, \ref{sec:grad_norm}, and \ref{sec:grad_proj}, we first obtain foreground masks with very weak supervision. Figure  \ref{fig:unsupervised_foreground} shows an image, its ground truth and the foreground segmentation provided by clustering. Note that a rough segmentation is enough to be useful for these approaches.
